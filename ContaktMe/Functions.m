...
+(void)createReminderNotificationForMedia:(NSString *)mediaId WithTitle:(NSString *)title onDate:(NSDate *)date UntilDate:(NSDate *)endDate {
    ...
    aNotification.fireDate = [date dateByAddingTimeInterval:-5*60];
    ...
    NSDictionary *dictId =  @{@"uid" : mediaId};


    aNotification.userInfo = dictId;
    ...
    if([eventStore respondsToSelector:@selector(requestAccessToEntityType:completion:)]) {
        ...
        [eventStore requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *error) {
            if (granted) {
                ...
                NSError *err;
                //BOOL result = [eventStore saveCalendar:calendar commit:YES error:&error]; //calendar

                [eventStore saveEvent:event span:EKSpanThisEvent commit:YES error:&err];
                NSString *savedEventId = event.eventIdentifier;

                [defaults setObject:savedEventId forKey:@"event"];

                //[createdReminders addObject:savedEventId];

                if(err)
                    NSLog(@"No se guardo en el Calendario el evento %@ El error es %@",title, err);

            }
            else // if he does not allow
            {
                [[[UIAlertView alloc]initWithTitle:nil message:@"ERROR" delegate:nil cancelButtonTitle:NSLocalizedString(@"No tienes permisos para acceder al calendario.", nil)  otherButtonTitles: nil] show];
                return;
            }
        }];
    }
    ...
    [defaults setObject:[NSArray arrayWithArray:createdReminders] forKey:@"reminders"];
    [defaults synchronize];
}
